...
#define APKEY_RCC		RCC_APB2Periph_GPIOA
#define APKEY_GPIO  	GPIOA
#define APKEY_PIN  	(GPIO_Pin_8)
...
unsigned char APKEY_EVENT = 0;	//AP按键事件
//外部中断9_5服务程序
void EXTI9_5_IRQHandler(void)
{
	...
	if(APKEY_Status == 0)
	{
		printf("APKEY_Status_Event\n");
		APKEY_EVENT = 1;
		EXTI_ClearITPendingBit(EXTI_Line8); //清除LINE9上的中断标志位 
	}
}
...
{ 
 	...
 	RCC_APB2PeriphClockCmd(APKEY_RCC,ENABLE);//使能portB的时钟
	
	GPIO_InitStructure.GPIO_Pin  = APKEY_PIN;//PB9
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; //设置成上拉输入
 	GPIO_Init(APKEY_GPIO, &GPIO_InitStructure);//初始化PB9
...
void APEXTIX_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO,ENABLE);	//使用复用时钟功能
	...
	//GPIOB.9 中断线以及中断初始化配置   下降沿触发
 	...
 	EXTI_InitStructure.EXTI_Line=EXTI_Line8;	//KEY_RESET
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;	
 	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;
 	EXTI_InitStructure.EXTI_LineCmd = ENABLE;
 	EXTI_Init(&EXTI_InitStructure);	 	//根据EXTI_InitStruct中指定的参数初始化外设EXTI寄存器
