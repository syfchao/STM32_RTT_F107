...
{
	unsigned short packLen = 0,uidNum = 0,len=0,i = 0;
	char temp[6] = {'\0'};
				...
				for(j=0; j<MAXINVERTERCOUNT; j++, curinverter++)
				{
					if(!memcmp(curinverter->id,UID,12))
					{
						...
						for( k = 0;k<sendNum;k++)
						{
							...
							if(ret > 0)
							{
								ResponseZigbeeTransimission(1,curinverter->id,ret*2,(unsigned char *)data);
								break;
							}
						}
						if(ret<=0)
						{
							ResponseZigbeeTransimission(0,curinverter->id,ret*2,(unsigned char *)data);
						}
					}
				}
			}	
