...
int response_ZigBeeChannel_Result(const char *recvbuffer, char *sendbuffer)
{
	...
	{
		/* ECU Message */
		ecu_ZigBeeChannel_msg(sendbuffer, (ecu.count + 1), recvbuffer);
		//拼接ECU信道UID全部写0
		inverter_ZigBeeChannel_msg(sendbuffer,"000000000000",ecu.channel,ecu.panid,0);
	
		for(i = 0; i < ecu.count;i++)
		{
			inverter_ZigBeeChannel_msg(sendbuffer,inverter[i].id,0,0,inverter[i].shortaddr);		
		}
	}
	return 0;
}
