...
//extern int caltype;		//璁＄畻鏂瑰紡锛孨A鐗堝拰闈濶A鐗堢殑鍖哄埆
extern ecu_info ecu;
...
int write_gfdi_status(inverter_info *firstinverter) 
{
    ...
    write_buff = malloc(MAXINVERTERCOUNT*INVARTER_STATUS_PER_LEN+INVARTER_STATUS_PER_OTHER);	//每台16个字节  20个字节作为预留
...
int save_gfdi_changed_result(inverter_info *firstinverter)
{
    ...
    strcat(gfdi_changed_result, ecu.id);					//ECU鐨処D
    strcat(gfdi_changed_result, "0000");					//閫嗗彉鍣ㄤ釜鏁?
    strcat(gfdi_changed_result, "00000000000000");		//鏃堕棿鎴筹紝璁剧疆閫嗗彉鍣ㄥ悗杩斿洖鐨勭粨鏋滀腑鏃堕棿鎴充负0
    strcat(gfdi_changed_result, "END");					//鍥哄畾鏍煎紡
...
int save_turn_on_off_changed_result(inverter_info *firstinverter)
{
    ...
    strcat(turn_on_off_changed_result, ecu.id);					//ECU鐨処D
    strcat(turn_on_off_changed_result, "0000");					//閫嗗彉鍣ㄤ釜鏁?
    strcat(turn_on_off_changed_result, "00000000000000");		//鏃堕棿鎴筹紝璁剧疆閫嗗彉鍣ㄥ悗杩斿洖鐨勭粨鏋滀腑鏃堕棿鎴充负0
    strcat(turn_on_off_changed_result, "END");					//鍥哄畾鏍煎紡
