...
int resolvedata_600_new(char *data, struct inverter_info_t *inverter)
{
    ...
    if(0==seconds)//é˜²æ­¢ç©ºæ•°æ®ä¼ è¿‡æ¥å¯¼è‡´ä¸»ç¨‹åºå‡ºé”?
    {
        inverter->op = 0;
        inverter->opb = 0;
    }
    ...
    if(inverter->curacctime > 600)		//é˜²æ­¢æ—©ä¸Šèµ·æ¥åŠŸçŽ‡çˆ†ç‚¹ï¼Œæ‰€ä»¥åœ¨å¯åŠ¨10åˆ†é’ŸåŽæ‰é‡‡ç”¨æ—¶é—´å·®çš„åŠŸçŽ‡è®¡ç®—æ¨¡å¼ï¼ŒZK
    {
        inverter->op = inverter->curgeneration * 1000.0 * 3600.0 / seconds;
        inverter->opb = inverter->curgenerationb * 1000.0 * 3600.0 / seconds;
    }
    ...
    inverter->status_web[25]=((data[24]>>2)&0x01)+0x30;		//ååˆ†é’Ÿå¹³å‡ä¿æŠ?
    ...
    inverter->status[0]=inverter->status_web[1];			//äº¤æµç«¯é¢‘çŽ‡æ¬ é¢?
    inverter->status[1]=inverter->status_web[0];			//äº¤æµç«¯é¢‘çŽ‡è¿‡é¢?
    inverter->status[2]=inverter->status_web[23];			//äº¤æµç«¯ç”µåŽ‹è¿‡åŽ?
    inverter->status[3]=inverter->status_web[24];			//äº¤æµç«¯ç”µåŽ‹æ¬ åŽ?
    inverter->status[4]=inverter->status_web[8];			//ç›´æµç«¯Aè·¯è¿‡åŽ?
    inverter->status[5]=inverter->status_web[9];			//ç›´æµç«¯Aè·¯æ¬ åŽ?
    inverter->status[6]=inverter->status_web[16];			//æ¸©åº¦è¿‡é«˜
    inverter->status[7]=inverter->status_web[17];			//GFDI
    ...
    inverter->statusb[0]=inverter->status_web[1];			//äº¤æµç«¯é¢‘çŽ‡æ¬ é¢?
    inverter->statusb[1]=inverter->status_web[0];			//äº¤æµç«¯é¢‘çŽ‡è¿‡é¢?
    inverter->statusb[2]=inverter->status_web[23];			//äº¤æµç«¯ç”µåŽ‹è¿‡åŽ?
    inverter->statusb[3]=inverter->status_web[24];			//äº¤æµç«¯ç”µåŽ‹æ¬ åŽ?
    inverter->statusb[4]=inverter->status_web[10];			//ç›´æµç«¯Bè·¯è¿‡åŽ?
    inverter->statusb[5]=inverter->status_web[11];			//ç›´æµç«¯Bè·¯æ¬ åŽ?
    inverter->statusb[6]=inverter->status_web[16];			//æ¸©åº¦è¿‡é«˜
    inverter->statusb[7]=inverter->status_web[17];		//GFDI
...
int resolvedata_600_60(char *data, struct inverter_info_t *inverter)
{
    ...
    for(i=0;i<46;i++)
        inverter->status_web[i] = 0x30;
    inverter->status_web[0]='0';//((data[25]>>6)&0x01)+0x30;		//AC Frequency exceeding Range 1bit"äº¤æµé¢‘çŽ‡å¤ªé«˜"
    inverter->status_web[1]='0';//((data[25]>>7)&0x01)+0x30;			//AC Frequency under Range 1bit"äº¤æµé¢‘çŽ‡å¤ªä½Ž";
    inverter->status_web[2]='0';//((data[25]>>5)&0x01)+0x30;		//AC-A Voltage exceeding Range 1bit"Aè·¯äº¤æµç”µåŽ‹å¤ªé«?
    inverter->status_web[3]='0';//((data[25]>>4)&0x01)+0x30;		//AC-A Voltage under Range 1bit"Aè·¯äº¤æµç”µåŽ‹å¤ªä½?
    inverter->status_web[4]='0';//((data[25]>>5)&0x01)+0x30;		//Bè·¯äº¤æµç”µåŽ‹å¤ªé«?
    inverter->status_web[5]='0';//((data[25]>>4)&0x01)+0x30;		//Bè·¯äº¤æµç”µåŽ‹å¤ªä½?
    ...
    inverter->status_web[8]=((data[25]>>2)&0x01)+0x30;			//DC-A Voltage Too High 1bitAè·¯ç›´æµç”µåŽ‹å¤ªé«?
    inverter->status_web[9]=((data[25]>>3)&0x01)+0x30;		//DC-A Voltage Too Low 1bitAè·¯ç›´æµç”µåŽ‹å¤ªä½?
    inverter->status_web[10]=((data[25]>>4)&0x01)+0x30;		//DC-B Voltage Too High 1bitBè·¯ç›´æµç”µåŽ‹å¤ªé«?
    inverter->status_web[11]=((data[25]>>5)&0x01)+0x30;		//DC-B Voltage Too Low 1bitBè·¯ç›´æµç”µåŽ‹å¤ªä½?
    //	inverter->status_web[12]=((data[24]>>4)&0x01)+0x30;		//
    //	inverter->status_web[13]=((data[24]>>5)&0x01)+0x30;		//
    //	inverter->status_web[14]=((data[24]>>6)&0x01)+0x30;		//
    //	inverter->status_web[15]=data[48]+0x30;				//gfdiçŠ¶æ€?
    inverter->status_web[16]=((data[23]>>3)&0x01)+0x30;			//Over Critical Temperature 1bitè¶…å‡ºæ¸©åº¦èŒƒå›´
    inverter->status_web[17]=((data[23]>>1)&0x01)+0x30;		//GFDI"GFDIé”ä½"
    inverter->status_web[18]=((data[23]>>2)&0x01)+0x30;		//Remote-shut"è¿œç¨‹å…³é—­"
    inverter->status_web[19]=((data[23]>>4)&0x01)+0x30;		//AC-Disconnect"äº¤æµæ–­å¼€
    inverter->status_web[21]=((data[24]>>3)&0x01)+0x30;		//"ä¸»åŠ¨å­¤å²›ä¿æŠ¤
    inverter->status_web[22]=((data[24]>>4)&0x01)+0x30;		//"CPä¿æŠ¤
    inverter->status_web[23]='0';		//äº¤æµç”µåŽ‹å¤ªé«˜
    inverter->status_web[24]='0';		//äº¤æµç”µåŽ‹å¤ªä½Ž

    inverter->status_web[25]=((data[23]>>0)&0x01)+0x30;		//ååˆ†é’Ÿå¹³å‡ä¿æŠ?
    inverter->status_web[26]=((data[24]>>7)&0x01)+0x30;		//æ¯çº¿æ¬ åŽ‹ä¿æŠ¤
    inverter->status_web[27]=((data[24]>>5)&0x01)+0x30;		//æ¯çº¿è¿‡åŽ‹ä¿æŠ¤
    inverter->status_web[28]=((data[24]>>6)&0x01)+0x30;		//ç»§ç”µå™¨æ•…éšœä¿æŠ?

    inverter->status_web[29]=((data[26]>>0)&0x01)+0x30;		//äº¤æµå¤–å›´é¢‘çŽ‡å¤ªé«˜
    inverter->status_web[30]=((data[26]>>1)&0x01)+0x30;		//äº¤æµå¤–å›´é¢‘çŽ‡å¤ªä½Ž
    inverter->status_web[31]=((data[26]>>2)&0x01)+0x30;		//äº¤æµå†…å›´é¢‘çŽ‡å¤ªé«˜
    inverter->status_web[32]=((data[26]>>3)&0x01)+0x30;		//äº¤æµå†…å›´é¢‘çŽ‡å¤ªä½Ž
    inverter->status_web[33]=((data[26]>>4)&0x01)+0x30;		//äº¤æµå¤–å›´ç”µåŽ‹å¤ªé«˜
    inverter->status_web[34]=((data[26]>>5)&0x01)+0x30;		//äº¤æµå¤–å›´ç”µåŽ‹å¤ªä½Ž
    inverter->status_web[35]=((data[26]>>6)&0x01)+0x30;		//äº¤æµå†…å›´ç”µåŽ‹å¤ªé«˜
    inverter->status_web[36]=((data[26]>>7)&0x01)+0x30;		//äº¤æµå†…å›´ç”µåŽ‹å¤ªä½Ž
    inverter->status_web[37]=((data[25]>>0)&0x01)+0x30;		//äº¤æµå†…å†…å›´ç”µåŽ‹å¤ªé«?
    inverter->status_web[38]=((data[25]>>1)&0x01)+0x30;		//äº¤æµå†…å†…å›´ç”µåŽ‹å¤ªä½?
    inverter->status_web[39]='0';							//Cè·¯ç›´æµç”µåŽ‹å¤ªé«?
    inverter->status_web[40]='0';							//Cè·¯ç›´æµç”µåŽ‹å¤ªä½?
    inverter->status_web[41]='0';							//Dè·¯ç›´æµç”µåŽ‹å¤ªé«?
    inverter->status_web[42]='0';							//Dè·¯ç›´æµç”µåŽ‹å¤ªä½?
    inverter->status_web[43]=((data[24]>>2)&0x01)+0x30;		//å‚æ•°è¯»å–å¤±è´¥
    inverter->status_web[44]=((data[23]>>5)&0x01)+0x30;		//äº¤æµæœ€å¤–å›´ç”µåŽ‹å¤ªé«˜
    inverter->status_web[45]=((data[23]>>6)&0x01)+0x30;		//äº¤æµæœ€å¤–å›´ç”µåŽ‹å¤ªä½Ž
...
int resolvedata_1200(char *data, struct inverter_info_t *inverter)
{
    ...
    for(i=0;i<44;i++)
        inverter->status_web[i] = 0x30;
    inverter->status_web[0]='0';		//AC Frequency exceeding Range 1bit"½»Á÷ÆµÂÊÌ«¸ß"
    inverter->status_web[1]='0';		//AC Frequency under Range 1bit"½»Á÷ÆµÂÊÌ«µÍ";
    ...
    inverter->status_web[8]=((data[25]>>2)&0x01)+0x30;		//DC-A Voltage Too High 1bitAÂ·Ö±Á÷µçÑ¹Ì«¸ß	OK
    inverter->status_web[9]=((data[25]>>3)&0x01)+0x30;		//DC-A Voltage Too Low 1bitAÂ·Ö±Á÷µçÑ¹Ì«µÍ	OK
    inverter->status_web[10]=((data[25]>>4)&0x01)+0x30;		//DC-B Voltage Too High 1bitBÂ·Ö±Á÷µçÑ¹Ì«¸ß	OK
    inverter->status_web[11]=((data[25]>>5)&0x01)+0x30;		//DC-B Voltage Too Low 1bitBÂ·Ö±Á÷µçÑ¹Ì«µÍ	OK
    //	inverter->status_web[12]=((data[24]>>4)&0x01)+0x30;		//
    //	inverter->status_web[13]=((data[24]>>5)&0x01)+0x30;		//
    //	inverter->status_web[14]=((data[24]>>6)&0x01)+0x30;		//
    //	inverter->status_web[15]=data[48]+0x30;					//gfdi×´Ì¬
    inverter->status_web[16]=((data[23]>>3)&0x01)+0x30;		 //Over Critical Temperature 1bit³¬³öÎÂ¶È·¶Î§
    inverter->status_web[17]=((data[23]>>1)&0x01)+0x30;		//GFDI"GFDIËø×¡"
    inverter->status_web[18]=((data[23]>>2)&0x01)+0x30;		//Remote-shut"Ô¶³Ì¹Ø±Õ"
    inverter->status_web[19]=((data[23]>>4)&0x01)+0x30;		//AC-Disconnect"½»Á÷¶Ï¿ª
    inverter->status_web[21]=((data[24]>>3)&0x01)+0x30;		//"Ö÷¶¯¹Âµº±£»¤
    inverter->status_web[22]=((data[24]>>4)&0x01)+0x30;		//"CP±£»¤
    inverter->status_web[23]='0';		//½»Á÷µçÑ¹Ì«¸ß
    inverter->status_web[24]='0';		//½»Á÷µçÑ¹Ì«µÍ

    inverter->status_web[25]=((data[23])&0x01)+0x30;		//Ê®·ÖÖÓÆ½¾ù±£»¤
    inverter->status_web[26]=((data[24]>>7)&0x01)+0x30;;		//Ä¸ÏßÇ·Ñ¹±£»¤
    inverter->status_web[27]=((data[24]>>5)&0x01)+0x30;;		//Ä¸Ïß¹ýÑ¹±£»¤
    inverter->status_web[28]=((data[24]>>6)&0x01)+0x30;;		//¼ÌµçÆ÷¹ÊÕÏ±£»¤
    inverter->status_web[29]=((data[26])&0x01)+0x30;		//½»Á÷ÍâÎ§ÆµÂÊÌ«¸ß	OK
    inverter->status_web[30]=((data[26]>>1)&0x01)+0x30;		//½»Á÷ÍâÎ§ÆµÂÊÌ«µÍ	OK
    inverter->status_web[31]=((data[26]>>2)&0x01)+0x30;		//½»Á÷ÄÚÎ§ÆµÂÊÌ«¸ß	OK
    inverter->status_web[32]=((data[26]>>3)&0x01)+0x30;		//½»Á÷ÄÚÎ§ÆµÂÊÌ«µÍ	OK
    inverter->status_web[33]=((data[26]>>4)&0x01)+0x30;		//½»Á÷ÍâÎ§µçÑ¹Ì«¸ß	OK
    inverter->status_web[34]=((data[26]>>5)&0x01)+0x30;		//½»Á÷ÍâÎ§µçÑ¹Ì«µÍ	OK
    inverter->status_web[35]=((data[26]>>6)&0x01)+0x30;		//½»Á÷ÄÚÎ§µçÑ¹Ì«¸ß	OK
    inverter->status_web[36]=((data[26]>>7)&0x01)+0x30;		//½»Á÷ÄÚÎ§µçÑ¹Ì«µÍ	OK
    inverter->status_web[37]=((data[25])&0x01)+0x30;		//½»Á÷ÄÚÄÚÎ§µçÑ¹Ì«¸ßOK
    inverter->status_web[38]=((data[25]>>1)&0x01)+0x30;		//½»Á÷ÄÚÄÚÎ§µçÑ¹Ì«µÍOK
    inverter->status_web[39]=((data[25]>>6)&0x01)+0x30;		//CÂ·Ö±Á÷µçÑ¹Ì«¸ß	OK
    inverter->status_web[40]=((data[25]>>7)&0x01)+0x30;		//CÂ·Ö±Á÷µçÑ¹Ì«µÍ	OK
    inverter->status_web[41]=((data[24])&0x01)+0x30;		//DÂ·Ö±Á÷µçÑ¹Ì«¸ß	OK
    inverter->status_web[42]=((data[24]>>1)&0x01)+0x30;		//DÂ·Ö±Á÷µçÑ¹Ì«¸ß	OK
    inverter->status_web[43]=((data[24]>>2)&0x01)+0x30;		//²ÎÊý¶ÁÈ¡Ê§°Ü		OK
...
int qs1200_status(struct inverter_info_t *inverter)
{
    ...
    if(('1' == inverter->status_web[19]) || ('1' == inverter->status_web[21]) || ('1' == inverter->status_web[22]) ||
        ...
        if( ('1' == inverter->status_web[19]) || ('1' == inverter->status_web[21]) || ('1' == inverter->status_web[22]) ||
            ...
            strcat(inverter->status_ema, "0");
        if('1' == inverter->status_web[16])			//ÎÂ¶È±£»¤
            strcat(inverter->status_ema, "1");
        else
            strcat(inverter->status_ema, "0");
        if(('1' == inverter->status_web[17])||('1' == inverter->status_web[28]))		//×é¼þ½ÓµØ¹ÊÕÏ±£»¤
            strcat(inverter->status_ema, "1");
        else
            strcat(inverter->status_ema, "0");
        if(('1' == inverter->status_web[18]) ||('1' == inverter->status_web[43]))			//¿ØÖÆ×´Ì¬±£»¤
            strcat(inverter->status_ema, "1");
        else
            strcat(inverter->status_ema, "0");
