...
float calsystemgeneration(struct inverter_info_t *inverter)		//ËÆ°ÁÆóÂΩìÂâç‰∏ÄËΩÆÁöÑÁ≥ªÁªüÂèëÁîµÈá?
{
    int i;
    float temp=0.0;
...
int transltgen(char *buff, float ltgen)		//Â¢ûÂä†ÂéÜÂè≤ÂèëÁîµÈá?
{
    int i,k;
    char ltgentemp[10]={'\0'};
    int temp = (int)ltgen;
...
int transdv(char *buff, float dv)				//Â¢ûÂä†ÈÄÜÂèòÂô®ÁöÑÁõ¥ÊµÅÁîµÂéã
{
    ...
    if(k>=0)
    {
        for(i=0; i<k; i++)		//Áõ¥ÊµÅÁîµÂéãÂç?‰∏™Â≠óËäÇÔºå‰∏éEMAÂçèËÆÆËßÑÂÆö
        {
            strcat(buff, "0");
        }
        strcat(buff, dvtemp);
...
int transdi(char *buff, float di)				//Â¢ûÂä†ÈÄÜÂèòÂô®ÁöÑÁõ¥ÊµÅÁîµÊµÅ
{
    ...
    if(k>=0)
    {
        for(i=0; i<k; i++)		//Áõ¥ÊµÅÁîµÊµÅÂç?‰∏™Â≠óËäÇÔºå‰∏éEMAÂçèËÆÆËßÑÂÆö
        {
            strcat(buff, "0");
        }
        strcat(buff, ditemp);
...
int transstatus(char *buff, char *status)		//Â¢ûÂä†ÈÄÜÂèòÂô®ÁöÑÁä∂ÊÄ?
{
...
int transcurgen(char *buff, float gen)		//Â¢ûÂä†ÈÄÜÂèòÂô®ÁöÑÂΩìÂâç‰∏ÄËΩÆÂèëÁîµÈáè
{
    ...
    else									//Â¶ÇÊûúÈïøÂ∫¶Ë∂ÖËøáÊúÄÂ§ßÈïøÂ∫¶ÔºåÂæóÂá∫ÂÄº‰∏∫Ë¥üÂÄºÔºåÂàôÁõ¥Êé•ÊãºÊé•CURGENLENGTH‰∏™Â≠óËä?
    {
...
int protocol_APS18(struct inverter_info_t *firstinverter, char *sendcommanddatetime)
{
    ...
    for(i=0; (i<MAXINVERTERCOUNT)&&(12==strlen(inverter->id)); i++)
    {
        if(1 == inverter->inverterstatus.dataflag)
        {
            ...
            else if((7 == inverter->model)||(8 == inverter->model))	//YC600ª˙–Õ
            {
                strcat(buff, inverter->id);
                strcat(buff, "07");
                transgridvolt(buff, inverter->gv);
...
//  device,eve,data\n
int saveevent(inverter_info *inverter, char *sendcommanddatatime)			//‰øùÂ≠òÁ≥ªÁªüÂΩìÂâç‰∏ÄËΩÆÂá∫Áé∞ÁöÑ7Áßç‰∫ã‰ª?Âè™‰øùÂ≠òÊúÄÊñ∞‰∏ÄËΩÆÁöÑÁä∂ÊÄ?
{
    int i=0,j =0;
